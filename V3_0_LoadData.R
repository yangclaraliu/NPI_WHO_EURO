...
oxford_data       <- read_csv("data/NPI_EURO.csv") 
vaccine_data      <- read_csv("data/VAC_EURO.csv")
...
# As of October 20th 2021, there is no missing data from the period 2020-01-01 to 2020-07-05
oxford_data_missing %>%
...
vaccine_data <- vaccine_data %>%
  rename(cnt = iso_code, country = location) %>% 
  group_by(country) %>% 
...
unique(policy_data$cnt) # n = 48,  rm(Turkmenistan, Armenia, Montenegro, North Macedonia & Malta)
unique(rt_estimates$cnt) # n = 52, rm(Turkmenistan)
unique(vaccine_data$cnt) # n = 52, rm(Turkmenistan)

intersect(unique(policy_data$cnt), unique(rt_estimates$cnt)) %>% .[!is.na(.)]
