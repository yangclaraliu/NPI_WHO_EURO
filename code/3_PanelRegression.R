...
p_tab %>% 
  ...
  geom_point(aes(shape = sig, color = cat), size = 3) +
  ...
  model_all_list[[i]] %>% 
    ...
    mutate(sig = if_else(`Pr...t..` <= 0.05, T, F),
           ...
           phase = factor(phase,
                          levels = c("wildtype", "Alpha", "Delta", "Omicron"),
                          labels = c("Wiletype phase", "Alpha phase", "Delta phase", "Omicron phase"))) %>% 
    left_join(policy_dic_V, by = "policy_code") -> p_tab_list[[i]]
...
for(i in 1:5){
  ...
  ggsave(paste0("figs/manuscript_fig4_0.", i, ".png"),
         p_save,
         height = 18, width = 10)
