...
gen_regress_data <- function(scen){
  ...
  if(scen == "s1_W") tmp <- joined$s1_W %>% 
      ...
      select(-c("V_18_60_adj", "V_60_adj", "V_tot_adj"))
  ...
  # S2
  if(scen == "s2_W") tmp <- joined$s2_W %>% 
      filter(!country %in% cnt_remove) %>% 
      select(-c("V_18_60_adj", "V_60_adj", "V_tot_adj"))
  
  if(scen == "s2_A") tmp <- joined$s2_A %>% 
      filter(!country %in% cnt_remove) %>% 
      select(-c("V_18_60_adj", "V_60_adj", "V_tot_adj"))
  
  if(scen == "s2_D") tmp <- joined$s2_D %>% 
      filter(!country %in% cnt_remove) %>% 
      select(-c("V_18_60_adj", "V_60_adj", "V_tot_adj"))
  
  # S3
  if(scen == "s3_W") tmp <- joined$s3_W %>% 
      filter(!country %in% cnt_remove) %>% 
      select(-c("V_18_60_adj", "V_60_adj", "V_tot_adj"))
  
  if(scen == "s3_A") tmp <- joined$s3_A %>% 
      filter(!country %in% cnt_remove) %>% 
      select(-c("V_18_60_adj", "V_60_adj", "V_tot_adj"))
  
  if(scen == "s3_D") tmp <- joined$s3_D %>% 
      filter(!country %in% cnt_remove) %>% 
      select(-c("V_18_60_adj", "V_60_adj", "V_tot_adj"))
  
  # S4
  if(scen == "s4_W") tmp <- joined$s4_W %>% 
      filter(!country %in% cnt_remove) %>% 
      select(-c("V_18_60_adj", "V_60_adj", "V_tot_adj"))
  
  if(scen == "s4_A") tmp <- joined$s4_A %>% 
      filter(!country %in% cnt_remove) %>% 
      select(-c("V_18_60_adj", "V_60_adj", "V_tot_adj"))
  
  if(scen == "s4_D") tmp <- joined$s4_D %>% 
      filter(!country %in% cnt_remove) %>% 
      select(-c("V_18_60_adj", "V_60_adj", "V_tot_adj"))
...
save(res_tab, res_tab_forward, file = "results/res_tab_V5.rdata")
...
AIC_panel <- lapply(1:36, function(x) tmp[[x]] %>% do.call("rbind", .)) %>% # change 1:36 to the number of sets
  map(mutate, model = 1:n()) %>% 
  bind_rows(.id = "set") 

lapply(lapply(res_tab$select,"[[",2),"[[","var_combo") -> tmp 

BIC_panel <- lapply(1:36, function(x) tmp[[x]] %>% do.call("rbind", .)) %>% 
  map(mutate, model = 1:n()) %>% 
  bind_rows(.id = "set")
...
var_select_res %>%
  ...
  geom_text(aes(x = lags,
                y = var,
                label = p_lab)) +
  ggh4x::facet_nested(virus ~ criterion + scen_grp)  +
  geom_point(aes(x = 1, y = 5, color = cat), alpha = 0) +
  scale_color_manual(values = c('#a6cee3',
                                '#1f78b4',
                                '#b2df8a',
                                '#33a02c',
                                '#8856a7')) +
  geom_segment(data = data.frame(y = c(0.5, 0.5),
                                 yend = c(15.5, 15.5),
                                 x = c(as.numeric(lag_for_plot[i])-0.5,as.numeric(lag_for_plot[i])+0.5),
                                 xend = c(as.numeric(lag_for_plot[i])-0.5,as.numeric(lag_for_plot[i])+0.5)),
               aes(x = x, xend = xend, y = y, yend = yend)) +
  theme_cowplot() +
  scale_y_continuous(breaks = seq(1,15,1),
                     labels = joined$policy_dic %>% 
  ...
  xlim(c(as.numeric(lag_for_plot[i])-0.5,as.numeric(lag_for_plot[i])+0.5))+ #
  
  labs(x = "Sensitivity Analysis Scenario", 
       y = "", 
       fill = "", 
       color = "Intervention Category",
       title = paste0("Temporal lag = ", optim_lag_name[i])) +
  guides(color = guide_legend(override.aes = list(alpha = 1,
                                                  size = 3),
                              nrow = 2))
...
# Plot and save significance tables
plot_sig_tab(1)
ggsave("figs/EURO_V5/fig4_sen_1.png",
       width = 15,
       height = 10) 

plot_sig_tab(2)
ggsave("figs/EURO_V5/fig4_sen_2.png",
       width = 15,
       height = 10) 

plot_sig_tab(3)
ggsave("figs/EURO_V5/fig4_sen_3.png",
       width = 15,
       height = 10) 
