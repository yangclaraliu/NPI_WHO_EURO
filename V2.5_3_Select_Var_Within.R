...
country_list <- unique(joined$hi$cnt) %>% 
  enframe(value = "cnt") %>% 
  mutate(country_name = countrycode::countrycode(cnt, "iso3c", "country.name"),
         region = "WHO EUROPE") 


gen_regress_data <- function(scen, min_date, max_date){
...
data_low <- gen_regress_data(scen = "Low",  min_date = "2020-01-01", max_date = "2020-11-30")
...
expand.grid(scen = c("Mid", "High"), 
                        optim_lag = c(-1, -5, -10),
                        VOC = c("Original", "Alpha", "Delta")) %>% 
  left_join(., VOC_time, by = "VOC") %>% 
  #select(-VOC) %>% 
...
AIC_panel <- lapply(1:18, function(x) tmp[[x]] %>% do.call("rbind", .)) %>% 
  ...
  mutate(lags = case_when(optim_lag == -1 ~ 1,
         ...
         scen = factor(scen, 
                       ...
                       labels = c("Any Efforts",
                                  ...
                                  "Max. Efforts")),
         VOC = factor(VOC,
                           levels = c("Original", "Alpha", "Delta"))) -> var_select_res
...
here("figs/EURO_0", "fig4_v4.png") %>%
  ggsave(width = 16, height = 8, plot = p4)
...
eff_size <- chosen_models %>% 
  ...
  left_join(chosen %>% mutate(id = 1:n() %>% as.character),
            by = "id") %>% 
  left_join(res_tab[,1:5] %>% mutate(set = 1:n() %>% as.character), by = "set") %>% 
...
eff_size %>% 
  ...
  dplyr::select(id, set) %>% 
  distinct() %>% 
  left_join(res_tab[,1:5] %>% mutate(set = as.character(1:n())), by = "set")
...
eff_size %>% 
  ...
  ggh4x::facet_nested(scen + VOC ~ var + criterion,
