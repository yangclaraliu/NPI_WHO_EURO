...
res_tab_forward <- res_tab <- expand.grid(scen = c("High_W", "High_A", "High_D", "Mid_W", "Mid_A", "Mid_D"), 
                                          optim_lag = c(-1, -14, -28)) %>% 
  mutate(select = NA)
...
effect_data %>% 
  ...
  labs(y = "Effects",
       ...
       shape = "Effect Type",
       title = "Multi-level NPI strength") +
