oxford_data_latest <- read.table("data/OxCGRT_latest.txt", sep = ",") %>%  
  as.tibble() %>% 
  setNames(.[1,]) %>% 
  .[-1,] %>% 
...
oxford_data_latest %<>% 
  select(-ends_with("Flag"), -starts_with("V"), -CountryCode) %>% 
...
oxford_data_latest %<>% 
  left_join(dic_tmp, by = "CountryName") %>% 
  select(CountryName,iso3c,Date,policy_name,value) %>% 
  setNames(c("country","cnt","date","policy_name","value"))

write_rds(oxford_data_latest,"data/oxford_data_latest.rds")
